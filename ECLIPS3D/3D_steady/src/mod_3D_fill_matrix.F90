 ...
 ! --------------------- u evolution ------------------------------------
      DO i=1, nlong
        DO j=1,nlat
          DO k=1,nz
            IF (k==0) THEN
              !alpha = diffusion_u(u,ddu_ddl_on_u(u,i,j,k,dlambda),du_dphi_on_u(u,i,j,k,dphi), &
      ...
      DO i=1, nlong
        DO j=0,nlat
          DO k=1,nz
            IF (j==0) THEN
              !alpha=(-1)*diffusion_v(v,ddv_ddl_on_v(v,i,j,k,dlambda),dv_dphi_on_v(v,i,j,k,dphi), &
              !ddv_ddphi_on_v(v,i,j,k,dphi),dv_dr_on_v(v,i,j,k,dz),ddv_ddr_on_v(v,i,j,k,dz), &
              !Rho_v(i,j,k), dRho_dr_v(i,j,k), dv_dr_on_v(dRho_dr_v,i,j,k,dz),phi_v,z_u,i,j,k)
              alpha = v(i,j,k)*tdrag_v(i,j,k)
            ELSE IF (j==nlat) THEN
              !alpha=(-1)*diffusion_v(v,ddv_ddl_on_v(v,i,j,k,dlambda),dv_dphi_on_v(v,i,j,k,dphi), &
              !ddv_ddphi_on_v(v,i,j,k,dphi),dv_dr_on_v(v,i,j,k,dz),ddv_ddr_on_v(v,i,j,k,dz), &
              !Rho_v(i,j,k), dRho_dr_v(i,j,k), dv_dr_on_v(dRho_dr_v,i,j,k,dz),phi_v,z_u,i,j,k)
            ...
            ELSE
              ...
              !(-1)*diffusion_v(v,ddv_ddl_on_v(v,i,j,k,dlambda),dv_dphi_on_v(v,i,j,k,dphi), &
              !ddv_ddphi_on_v(v,i,j,k,dphi),dv_dr_on_v(v,i,j,k,dz),ddv_ddr_on_v(v,i,j,k,dz), &
              !Rho_v(i,j,k), dRho_dr_v(i,j,k), dv_dr_on_v(dRho_dr_v,i,j,k,dz),phi_v,z_u,i,j,k)
...
!---------------------------------------------------------------------------
  ...
  FUNCTION ddu_ddl_on_u(u,i,j,k,dlambda)
    ...
    IF (i==1) THEN
      ddu_ddl_on_u = (0.5*u(i+1,j,k)-0.5*u(i,j,k))/ &
      (dlambda*dlambda)
    ELSE IF (i==nlong) THEN
      ddu_ddl_on_u = (0.5*u(i-1,j,k)-0.5*u(i,j,k))/ &
      (dlambda*dlambda)
    ELSE
      ddu_ddl_on_u = (u(i+1,j,k)+u(i-1,j,k)-2*u(i,j,k))/ &
      (dlambda*dlambda)
    END IF
    RETURN
  END FUNCTION
  ...
  FUNCTION ddu_ddphi_on_u(u,i,j,k,dphi)
    ...
    IF(j==1) THEN
      ddu_ddphi_on_u = (0.5*u(i,j+1,k)-0.5*u(i,j,k))/ &
      (dphi*dphi)
    ELSE IF (j==nlat) THEN
      ddu_ddphi_on_u = (0.5*u(i,j-1,k)-0.5*u(i,j,k))/ &
      (dphi*dphi)
    ELSE
      ddu_ddphi_on_u = (u(i,j+1,k)+u(i,j-1,k)-2*u(i,j,k))/ &
      (dphi*dphi)
    END IF
  ...
  FUNCTION ddu_ddr_on_u(u,i,j,k,dr)
    ...
    IF(k==1) THEN
      ddu_ddr_on_u = (0.5*u(i,j,k+1)-0.5*u(i,j,k))/ &
      (dr*dr)
    ELSE IF (k==nz) THEN
      ddu_ddr_on_u = (0.5*u(i,j,k-1)-0.5*u(i,j,k))/ &
      (dr*dr)
  ...
  FUNCTION ddv_ddl_on_v(v,i,j,k,dlambda)
    ...
    IF (i==1) THEN
      ddv_ddl_on_v = (0.5*v(i+1,j,k)-0.5*v(i,j,k))/ &
      (dlambda*dlambda)
    ELSE IF (i==nlong) THEN
      ddv_ddl_on_v = (0.5*v(i-1,j,k)-0.5*v(i,j,k))/ &
      (dlambda*dlambda)
    ELSE
      ddv_ddl_on_v = (v(i+1,j,k)+v(i-1,j,k)-2*v(i,j,k))/ &
      (dlambda*dlambda)
    END IF
  ...
  FUNCTION ddv_ddphi_on_v(v,i,j,k,dphi)
    ...
    ELSE IF (j==nlat) THEN
      ...
      (dphi*dphi)
    ELSE
      ddv_ddphi_on_v = (v(i,j+1,k)+v(i,j-1,k)-2*v(i,j,k))/ &
      (dphi*dphi)
    END IF
  ...
  FUNCTION ddv_ddr_on_v(v,i,j,k,dr)
    ...
    IF(k==1) THEN
      ddv_ddr_on_v = (0.5*v(i,j,k+1)-0.5*v(i,j,k))/ &
      (dr*dr)
    ELSE IF (k==nz) THEN
      ddv_ddr_on_v = (0.5*v(i,j,k-1)-0.5*v(i,j,k))/ &
      (dr*dr)
    ELSE
      ddv_ddr_on_v = (v(i,j,k+1)+v(i,j,k-1)-2*v(i,j,k))/ &
      (dr*dr)
    END IF
    RETURN
  END FUNCTION
  ...
  FUNCTION diffusion_u(u,ddu_ddl,du_dphi,ddu_ddphi,du_dr,ddu_ddr,&
    ...
    du_dphi,ddu_ddphi,du_dr,ddu_ddr, Rho, dRho_dr, ddRho_ddr
    ...
    diffusion_u = 1./COS(phi_u(j)) * (1.0E-8*(ddu_ddl - &
    SIN(phi_u(j))*du_dphi + COS(phi_u(j))*ddu_ddphi) + &
    1.0E-12*(u(i,j,k)* (-2.* (z_u(k))/Rho * dRho_dr - 2.d0 + &
    2.* ((z_u(k))/Rho * dRho_dr)**2 - &
    (z_u(k))**2./Rho * ddRho_ddr ) + &
    du_dr*(2.*(z_u(k))- &
    2* (z_u(k))**2/Rho *dRho_dr) + &
    (z_u(k))**2*ddu_ddr ) )
  ...
  FUNCTION diffusion_v(v,ddv_ddl,dv_dphi,ddv_ddphi,dv_dr,ddv_ddr,&
           Rho, dRho_dr,ddRho_ddr,phi_v,z_u,i,j,k)

    DOUBLE PRECISION, DIMENSION(nlong,0:nlat,nz), INTENT(IN) :: v

    DOUBLE PRECISION, INTENT(IN) :: phi_v(0:nlat), z_u(nz)

    DOUBLE PRECISION, INTENT(IN) ::  ddv_ddl, &
    dv_dphi,ddv_ddphi,dv_dr,ddv_ddr, Rho, dRho_dr, ddRho_ddr

    INTEGER, INTENT(IN) :: i,j,k

    DOUBLE PRECISION :: diffusion_v

    diffusion_v = 1/COS(phi_v(j)) * (1.0E-8*(ddv_ddl - &
    SIN(phi_v(j))*dv_dphi + COS(phi_v(j))*ddv_ddphi) + &
    1.0E-12*(v(i,j,k)* (-2.* (z_u(k))/Rho * dRho_dr - 2 + &
    2.* ((z_u(k))/Rho * dRho_dr)**2 - &
    (z_u(k))**2./Rho * ddRho_ddr ) + &
    dv_dr*(2.*(z_u(k))- &
    2* (z_u(k))**2/Rho *dRho_dr) + &
