 ...
 ! --------------------- u evolution ------------------------------------
      DO i=1, nlong
        DO j=1,nlat
          DO k=1,nz
            IF (k==0) THEN
              alpha = 0.d0!u(i,j,k)*tdrag_u(i,j,k)
            ELSE
              alpha=u(i,j,k)*((dUs_dl_u(i,j,k)-Us_u(i,j,k)*dRho_dl_u(i,j,k)/Rho_u(i,j,k)) &
              /(z_u(k)*DCOS(phi_u(j))) - Vs_u(i,j,k)*dRho_dphi_u(i,j,k)/(z_u(k)*Rho_u(i,j,k)) - &
              Ws_u(i,j,k)*dRho_dr_u(i,j,k)/Rho_u(i,j,k) + Ws_u(i,j,k)/z_u(k) - &
              DTAN(phi_u(j))*Vs_u(i,j,k)/z_u(k)) + &
              du_dl_on_u(u,i,j,k,dlambda)*Us_u(i,j,k)/(z_u(k)*DCOS(phi_u(j))) + &
              du_dphi_on_u(u,i,j,k,dphi)*Vs_u(i,j,k)/z_u(k) + &
              du_dr_on_u(u,i,j,k,dz)*Ws_u(i,j,k) + &
              v_on_u(v,i,j,k)*(dUs_dphi_u(i,j,k)/z_u(k)-2.d0*omega*DSIN(phi_u(j))- &
              Us_u(i,j,k)*DTAN(phi_u(j))/z_u(k) ) + &
              w_on_u(w,i,j,k)*(dUs_dr_u(i,j,k)+2.d0*omega*DCOS(phi_u(j))+Us_u(i,j,k)/z_u(k)) + &
        ...
        DO j=0,nlat
          DO k=1,nz
            ...
            ELSE IF (k==0) THEN
              alpha=0.d0
            ELSE
              alpha=u_on_v(u,i,j,k)*(dVs_dl_v(i,j,k)/(z_u(k)*DCOS(phi_v(j)))+ &
              2.d0*omega*DSIN(phi_v(j)) + &
              2.d0*DTAN(phi_v(j))*Us_v(i,j,k)/z_u(k)) + &
              v(i,j,k)*(-Us_v(i,j,k)*dRho_dl_v(i,j,k)/(z_u(k)*Rho_v(i,j,k)*DCOS(phi_v(j))) + &
              dVs_dphi_v(i,j,k)/z_u(k) - Vs_v(i,j,k)*dRho_dphi_v(i,j,k)/ &
              (z_u(k)*Rho_v(i,j,k)) - Ws_v(i,j,k)*dRho_dr_v(i,j,k)/Rho_v(i,j,k) + &
              Ws_v(i,j,k)/z_u(k)) + &
              dv_dl_on_v(v,i,j,k,dlambda)*(Us_v(i,j,k)/(z_u(k)*DCOS(phi_v(j)))) + &
              ...
              dp_dphi_on_v(p,i,j,k,dphi)*(1.d0/z_u(k)) + &
      ...
      DO i=1, nlong
        DO j=1,nlat
          DO k=1,nz
            ...
            (Rho_p(i,j,k)*z_u(k)*DCOS(phi_u(j))) ) + &
            du_dl_on_p(u,i,j,k,dlambda)*C_p_square(i,j,k)/(z_u(k)*DCOS(phi_u(j))) + &
            v_on_p(v,i,j,k) * ((dPs_dphi_p(i,j,k) - C_p_square(i,j,k)*dRho_dphi_p(i,j,k))/ &
            (Rho_p(i,j,k)*z_u(k)) - C_p_square(i,j,k)*DTAN(phi_u(j))/z_u(k) ) + &
            dv_dphi_on_p(v,i,j,k,dphi)*C_p_square(i,j,k)/z_u(k) + &
            w_on_p(w,i,j,k)*(C_p_square(i,j,k) * (2.d0/z_u(k) + N_p_square(i,j,k)/gz_u(k)) ) + &
            dw_dr_on_p(w,i,j,k,dz)*C_p_square(i,j,k) + &
            p(i,j,k)* (1.d0+kappa/(1.d0-kappa)) * (dUs_dl_p(i,j,k)/(z_u(k)*DCOS(phi_u(j))) + &
            dVs_dphi_p(i,j,k)/z_u(k) - Vs_p(i,j,k)*DTAN(phi_u(j))/z_u(k) + &
            dWs_dr_p(i,j,k) + 2.d0*Ws_p(i,j,k)/z_u(k) ) + &
            dp_dl_on_p(p,i,j,k,dlambda)*(Us_p(i,j,k)/ (z_u(k)*DCOS(phi_u(j))) ) + &
        ...
        DO j=1,nlat
          DO k=1,nz
            IF (k==nz) THEN
              alpha = 0.d0
            ELSE
              alpha = u_on_w(u,i,j,k)*(-2.d0*omega*DCOS(phi_u(j))-2.d0*Us_w(i,j,k)/z_w(k) + &
              dWs_dl_w(i,j,k)/(z_w(k)*DCOS(phi_u(j))) ) + &
              v_on_w(v,i,j,k)*(-2.d0*Vs_w(i,j,k)/z_w(k) + dWs_dphi_w(i,j,k)/z_w(k) ) + &
              w(i,j,k)*(-Us_w(i,j,k)*dRho_dl_w(i,j,k)/(Rho_w(i,j,k)*z_w(k)*DCOS(phi_u(j))) - &
              Vs_w(i,j,k)*dRho_dphi_w(i,j,k)/(z_w(k)*Rho_w(i,j,k)) + &
              dWs_dr_w(i,j,k) - Ws_w(i,j,k)*dRho_dr_w(i,j,k)/Rho_w(i,j,k) ) + &
              dw_dl_on_w(w,i,j,k,dlambda)*Us_w(i,j,k)/(z_w(k)*DCOS(phi_u(j))) + &
        ...
        DO j=1,nlat
          DO k=1,nz
            IF (k==nz) THEN
              alpha = 0.d0
            ELSE
            alpha = u_on_w(u,i,j,k)*(gz_w(k)*dThetas_dl_w(i,j,k)/(z_w(k)*DCOS(phi_u(j))* &
            Thetas_w(i,j,k))) + &
            v_on_w(v,i,j,k)*(gz_w(k)*dThetas_dphi_w(i,j,k)/(z_w(k)*Thetas_w(i,j,k))) + &
            w(i,j,k)*N_w_square(i,j,k) + &
            theta(i,j,k)*(Us_w(i,j,k)/(z_u(k)*DCOS(phi_u(j))) *  &
          ...
          DO k=1,nz
            IF (Ps_p(i,j,k)<P_high) THEN
              IF (Ps_p(i,j,k)<P_low) THEN
                logP=DLOG10(P_low/1.0D5)
              ELSE
                logP=DLOG10(Ps_p(i,j,k)/1.0D5)
              END IF
              trad_p(i,j,k)=1.d0/(10.d0**(5.4659686d0+1.4940124d0*logP+ &
              0.66079196d0*(logP**2.d0)+0.16475329d0*(logP**3.d0)+ &
              0.014241552d0*(logP**4.d0)) )
            ELSE
              trad_p(i,j,k)=0.d0
        ...
        DO j=1,nlat
          DO k=0,nz
           ...
           ELSE IF (Ps_w(i,j,k)<P_high) THEN
             trad_theta(i,j,k) =1.d0/ ( trad_bottom * (Ps_w(i,j,k) / P_high) ** &
             (DLOG((1.d0/trad)/trad_bottom)/DLOG(P_low/P_high)) )
           ELSE
             trad_theta(i,j,k) =1.d0 / trad_bottom
           END IF
         END DO
...
!--------------------------------------------------------------------
  ...
  FUNCTION u_on_v(u,i,j,k)
    ...
    IF (j==nlat) THEN
      DO t=1,nlong
        u_on_v=u_on_v+u(t,j,k)*DCOS((lambda_min+(dble(t)-1.d0)*360.d0/dble(nlong)) &
        *pi/180.d0)/DBLE(nlong)
      END DO

    ELSE IF (j==0) THEN
      IF (i==nlong) THEN
        u_on_v=0.25d0*(2.5d0*u(i,1,k)-0.5d0*u(i,2,k)+ &
        2.5d0*u(1,1,k)-0.5d0*u(1,2,k))
      ELSE
        u_on_v=0.25d0*(2.5d0*u(i,1,k)-0.5d0*u(i,2,k)+ &
        2.5d0*u(i+1,1,k)-0.5d0*u(i+1,2,k))
      END IF

    ELSE IF (i==nlong) THEN
      u_on_v=0.25d0*(u(i,j,k)+u(i,j+1,k)+u(1,j,k)+u(1,j+1,k))
  ...
  FUNCTION u_on_w(u,i,j,k)
    ...
    IF (k==0) THEN
      IF (i==nlong) THEN
        u_on_w=0.25d0*(2.5d0*u(i,j,1)-0.5d0*u(i,j,2)+ &
        2.5d0*u(1,j,1)-0.5d0*u(1,j,2))
      ELSE
        u_on_w=0.25d0*(2.5d0*u(i,j,1)-0.5d0*u(i,j,2)+ &
        2.5d0*u(i+1,j,1)-0.5d0*u(i+1,j,2))
      END IF

    ELSE IF (k==nz) THEN
      IF (i==nlong) THEN
        u_on_w=0.25d0*(2.5d0*u(i,j,nz)-0.5d0*u(i,j,nz-1)+ &
        2.5d0*u(1,j,nz)-0.5d0*u(i,j,nz-1))
      ELSE
        u_on_w=0.25d0*(2.5d0*u(i,j,nz)-0.5d0*u(i,j,nz-1)+ &
        2.5d0*u(i+1,j,nz)-0.5d0*u(i+1,j,nz-1))
      END IF
    ELSE
      IF (i==nlong) THEN
        u_on_w=0.25d0*(u(i,j,k)+u(i,j,k+1)+u(1,j,k)+u(1,j,k+1))
      ELSE
        u_on_w=0.25d0*(u(i,j,k)+u(i,j,k+1)+u(i+1,j,k)+u(i+1,j,k+1))
      END IF
    END IF
  ...
  FUNCTION du_dl_on_u(u,i,j,k,dlambda)
    ...
    ELSE IF (i==nlong) THEN
      du_dl_on_u=0.5d0*(1.5d0*u(i,j,k)-1.5d0*u(i-1,j,k))/dlambda
    ELSE
  ...
  FUNCTION du_dl_on_w(u,i,j,k,dlambda)
    ...
    ELSE IF (k==nz) THEN
      IF (i==nlong) THEN
        ...
        0.5d0*u(i,j,nz-1)-0.5d0*u(1,j,nz-1))/dlambda
      ELSE
        du_dl_on_w=0.5d0*(2.5d0*u(i+1,j,nz)-2.5d0*u(i,j,nz) + &
        0.5d0*u(i,j,nz-1)-0.5d0*u(i+1,j,nz-1))/dlambda
      END IF
    ELSE
      IF (i==nlong) THEN
        du_dl_on_w=0.5d0*(u(1,j,k+1)-u(i,j,k+1) + &
        u(1,j,k)-u(i,j,k))/dlambda
      ELSE
        du_dl_on_w=0.5d0*(u(i+1,j,k+1)-u(i,j,k+1) + &
  ...
  FUNCTION du_dphi_on_u(u,i,j,k,dphi)
    ...
    ELSE IF (j==nlat) THEN
      du_dphi_on_u=0.5d0*(1.5*u(i,j,k)-1.5d0*u(i,j-1,k))/dphi
    ELSE
      du_dphi_on_u=0.5d0*(u(i,j+1,k)-u(i,j-1,k))/dphi
    END IF
   RETURN
  END FUNCTION
  ...
  FUNCTION du_dr_on_u(u,i,j,k,dr)
    ...
    ELSE
      du_dr_on_u=0.5d0*(u(i,j,k+1)-u(i,j,k-1))/dr
    END IF
...
!--------------------------------------------------------------------
  ...
  FUNCTION v_on_u(v,i,j,k)
    ...
    ELSE
      v_on_u=0.25d0*(v(i-1,j-1,k)+v(i-1,j,k)+v(i,j-1,k)+v(i,j,k))
    END IF
  ...
  FUNCTION v_on_w(v,i,j,k)
    ...
    ELSE IF (k==nz) THEN
      v_on_w=0.25d0*(2.5d0*v(i,j-1,nz)-0.5d0*v(i,j-1,nz-1)+ &
            2.5d0*v(i,j,nz)-0.5d0*v(i,j,nz-1))
    ELSE
  ...
  FUNCTION dv_dphi_on_v(v,i,j,k,dphi)
    ...
    ELSE IF (j==nlat) THEN
      dv_dphi_on_v=0.5d0*(1.5d0*v(i,j,k)-1.5d0*v(i,j-1,k))/dphi
    ELSE
  ...
  FUNCTION dv_dphi_on_w(v,i,j,k,dphi)
    ...
    IF (k==0) THEN
      dv_dphi_on_w=0.5d0*(2.5d0*v(i,j,1)-2.5d0*v(i,j-1,1) + &
        0.5d0*v(i,j-1,2)-0.5d0*v(i,j,2))/dphi
    ELSE IF (k==nz) THEN
...
!--------------------------------------------------------------------  
  ...
  FUNCTION w_on_v(w,i,j,k)  
    ...
    ELSE IF (j==nlat) THEN
      w_on_v=0.25d0*(2.5d0*w(i,nlat,k-1)-0.5d0*w(i,nlat-1,k-1)+ &
        2.5d0*w(i,nlat,k)-0.5d0*w(i,nlat-1,k))
    ELSE
  ...
  FUNCTION dw_dr_on_w(w,i,j,k,dz)
    ...
    ELSE IF (k==nz) THEN
      dw_dr_on_w=0.5d0*(1.5d0*w(i,j,nz)-1.5d0*w(i,j,nz-1))/dz
    ELSE
...
!--------------------------------------------------------------------  
 ...
 FUNCTION p_on_v(p,i,j,k)
    ...
    ELSE IF (j==nlat) THEN
      p_on_v=0.5d0*(2.5d0*p(i,j,k)-0.5d0*p(i,j-1,k))
    ELSE
  ...
  FUNCTION dp_dl_on_p(p,i,j,k,dlambda)
    ...
    ELSE IF (i==nlong) THEN
      dp_dl_on_p=0.5d0*(1.5d0*p(i,j,k)-1.5d0*p(i-1,j,k))/dlambda
    ELSE
  ...
  FUNCTION dp_dphi_on_v(p,i,j,k,dphi)
    ...
    ELSE IF (j==nlat) THEN
      dp_dphi_on_v=0.5d0*(p(i,nlat,k)-p(i,nlat-1,k))/dphi
    ELSE
  ...
  FUNCTION dp_dphi_on_p(p,i,j,k,dphi)
    ...
    ELSE IF (j==nlat) THEN
      dp_dphi_on_p=0.5d0*(1.5d0*p(i,nlat,k)-1.5d0*p(i,nlat-1,k))/dphi
    ELSE
      dp_dphi_on_p=0.5d0*(p(i,j+1,k)-p(i,j-1,k))/dphi
    END IF
    RETURN
  END FUNCTION
  ...
  FUNCTION dp_dr_on_p(p,i,j,k,dr)
    ...
    ELSE IF (k==nz) THEN
      dp_dr_on_p=0.5d0*(1.5d0*p(i,j,k)-1.5d0*p(i,j,k-1))/dr
    ELSE
      dp_dr_on_p=0.5d0*(p(i,j,k+1)-p(i,j,k-1))/dr
    END IF
    RETURN
  END FUNCTION
  ...
  FUNCTION dp_dr_on_w(p,i,j,k,dz)
    ...
    ELSE IF (k==nz) THEN
      dp_dr_on_w=0.5d0*(p(i,j,nz)-p(i,j,nz-1))/dz
    ELSE
...
!---------------------------------------------------------------------------
  ...
  FUNCTION ddu_ddl_on_u(u,i,j,k,dlambda)
    ...
    DOUBLE PRECISION :: ddu_ddl_on_u

          IF(i==1) THEN
            ddu_ddl_on_u = (0.5d0*u(i+1,j,k)-0.5d0*u(i,j,k))/ &
            (dlambda*dlambda)
          ELSE IF (i==nlong) THEN
            ddu_ddl_on_u = (0.5d0*u(i-1,j,k)-0.5d0*u(i,j,k))/ &
            (dlambda*dlambda)
          ELSE
            ddu_ddl_on_u = (u(i+1,j,k)+u(i-1,j,k)-2.d0*u(i,j,k))/ &
            (dlambda*dlambda)
          END IF
        RETURN
  END FUNCTION
  ...
  FUNCTION ddu_ddphi_on_u(u,i,j,k,dphi)
    ...
    DOUBLE PRECISION :: ddu_ddphi_on_u

          IF(j==1) THEN
            ddu_ddphi_on_u = (0.5d0*u(i,j+1,k)-0.5d0*u(i,j,k))/ &
            (dphi*dphi)
          ELSE IF (j==nlat) THEN
            ddu_ddphi_on_u = (0.5d0*u(i,j-1,k)-0.5d0*u(i,j,k))/ &
            (dphi*dphi)
  ...
  FUNCTION ddu_ddr_on_u(u,i,j,k,dr)
    ...
    DOUBLE PRECISION :: ddu_ddr_on_u
          ...
          ELSE IF (k==nz) THEN
            ...
            (dr*dr)
          ELSE
            ddu_ddr_on_u = (u(i,j,k+1)+u(i,j,k-1)-2.d0*u(i,j,k))/ &
            (dr*dr)
          END IF
  ...
  FUNCTION ddv_ddl_on_v(v,i,j,k,dlambda)
    ...
    DOUBLE PRECISION :: ddv_ddl_on_v

          IF(i==1) THEN
            ddv_ddl_on_v = (0.5d0*v(i+1,j,k)-0.5d0*v(i,j,k))/ &
            (dlambda*dlambda)
          ELSE IF (i==nlong) THEN
            ddv_ddl_on_v = (0.5d0*v(i-1,j,k)-0.5d0*v(i,j,k))/ &
            (dlambda*dlambda)
  ...
  FUNCTION ddv_ddphi_on_v(v,i,j,k,dphi)
    ...
    DOUBLE PRECISION :: ddv_ddphi_on_v
          ...
          ELSE IF (j==nlat) THEN
            ...
            (dphi*dphi)
          ELSE
            ddv_ddphi_on_v = (v(i,j+1,k)+v(i,j-1,k)-2.d0*v(i,j,k))/ &
            (dphi*dphi)
          END IF
  ...
  FUNCTION ddv_ddr_on_v(v,i,j,k,dr)
    ...
    DOUBLE PRECISION :: ddv_ddr_on_v

          IF(k==1) THEN
            ddv_ddr_on_v = (0.5d0*v(i,j,k+1)-0.5d0*v(i,j,k))/ &
            (dr*dr)
          ELSE IF (k==nz) THEN
            ddv_ddr_on_v = (0.5d0*v(i,j,k-1)-0.5d0*v(i,j,k))/ &
            (dr*dr)
          ELSE
            ddv_ddr_on_v = (v(i,j,k+1)+v(i,j,k-1)-2.d0*v(i,j,k))/ &
            (dr*dr)
          END IF
...
! IL MANQUE LES DERIVEES DE RHO !!!!!


  FUNCTION diffusion_u(u, Rho_u, dRho_dr_u,K_lambda_u,K_phi_u,phi_u,z_u,i,j,k,dlambda,dphi)

    DOUBLE PRECISION, DIMENSION(nlong,nlat,nz), INTENT(IN) :: u, Rho_u, dRho_dr_u, K_lambda_u,K_phi_u
    DOUBLE PRECISION, INTENT(IN) :: phi_u(nlat), z_u(nz)

    DOUBLE PRECISION, INTENT(IN) :: dlambda, dphi
    ...
    DOUBLE PRECISION :: diffusion_u
          ...
          DSIN(phi_u(j))*du_dphi_on_u(u,i,j,k,dphi))
  ...
  FUNCTION diffusion_v(v,Rho_v,dRho_dr_v,K_lambda_v,K_phi_v,phi_v,z_u,i,j,k,dlambda,dphi)

    DOUBLE PRECISION, DIMENSION(nlong,0:nlat,nz), INTENT(IN) :: v, Rho_v, dRho_dr_v, &
    K_lambda_v,K_phi_v
    DOUBLE PRECISION, INTENT(IN) :: phi_v(0:nlat), z_u(nz),dlambda,dphi

    INTEGER, INTENT(IN) :: i,j,k

    DOUBLE PRECISION :: diffusion_v

          diffusion_v =K_lambda_v(i,j,k)/(z_u(k)**2*DCOS(phi_v(j))**2)*ddv_ddl_on_v(v,i,j,k,dlambda)+ &
          K_phi_v(i,j,k)/(z_u(k)**2*DCOS(phi_v(j)))*(DCOS(phi_v(j))*ddv_ddphi_on_v(v,i,j,k,dphi)-&
          DSIN(phi_v(j))*dv_dphi_on_v(v,i,j,k,dphi))
